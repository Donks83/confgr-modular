...
describe('following a product that changed', () => {
  ...
  it('never pulls in', () => {
    // A product getting SMALLER must not yank the camera closer: somebody who
    // zoomed out to see the whole run did that on purpose too.
    ...
    ctx.productRoot.clear();
    const small = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.55, 0.32));
    small.position.set(0, 0.28, 0);
